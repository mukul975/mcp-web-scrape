...
const robotsCache = new Map<string, { robots: any; timestamp: number }>();
 ...
 */
async function fetchRobots(robotsUrl: string): Promise<any> {
 ...
 */
export async function checkRobots(url: string, userAgent: string = config.userAgent): Promise<RobotsResult> {
 ...
 */
export async function checkUrlAllowed(url: string, bypassRobots: boolean = false): Promise<RobotsResult> {
  if (bypassRobots) {
    return {
      allowed: true,
 ...
 */
export function getRobotsCacheStats(): { size: number; entries: Array<{ url: string; timestamp: number }> } {
