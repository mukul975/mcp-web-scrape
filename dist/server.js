            ...
            switch (name) {
                ...
                case 'extract_social_media': {
                        ...
                        if (property && content) {
                            ...
                            Object.entries(platformPatterns).forEach(([platform, pattern]) => {
                                if (shouldExtract(platform) && pattern.test(content)) {
                                    ...
                                    if (!socialLinks[platform].some((link) => link.url === content)) {
                                        socialLinks[platform].push({
                                            url: content,
                                            username: match ? match[1] : '',
                    ...
                    const result = {
                        ...
                        totalLinksFound: Object.values(socialLinks).reduce((sum, links) => sum + links.length, 0),
                        detectionMethods: {
                            direct_link: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'direct_link').length,
                            iframe_widget: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'iframe_widget').length,
                            css_class: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'css_class').length,
                            contextual_mention: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'contextual_mention').length,
                            meta_tag: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'meta_tag').length,
                            json_ld: Object.values(socialLinks).flat().filter((link) => link.detectionMethod === 'json_ld').length,
                        },
                        fromCache: fetchResult.fromCache,
                    };
                    return {
                ...
                case 'extract_entities': {
                    ...
                    const result = {
                        ...
                        summary: {
                            totalEntities,
                            entitiesByType: Object.fromEntries(Object.entries(entities).map(([type, items]) => [type, items.length])),
                        },
                    };
                    return {
                        content: [
                ...
                case 'analyze_competitors': {
                    ...
                    const competitorAnalyses = await Promise.all(competitors.slice(0, 5).map(competitor => analyzeWebsite(competitor)));
                    // Compare metrics
                    const comparison = {
                        wordCount: {
                ...
                case 'extract_schema_markup': {
                    ...
                    const schemaTypes_found = new Set();
                ...
                case 'check_broken_links': {
                    ...
                    const linkStatuses = await Promise.all(uniqueLinks.map(async (link) => {
                        try {
                ...
                case 'analyze_page_speed': {
                    ...
                    if (resources.scripts > 10)
                        score -= 10;
                    if (estimatedTotalSize > 2000000)
                        score -= 20; // 2MB
                    // Add points for optimizations
                    if (performanceFactors.hasMinifiedCSS)
                        score += 5;
                    if (performanceFactors.hasMinifiedJS)
                        score += 5;
                    if (performanceFactors.hasLazyLoading)
                        score += 10;
                    if (performanceFactors.hasCDN)
                        score += 10;
                    if (performanceFactors.hasServiceWorker)
                        score += 15;
                    ...
                    $('img[src*="facebook.com"], img[src*="google-analytics.com"], img[src*="doubleclick.net"]').each((_, element) => {
                        ...
                        if (src) {
                            ...
                            if (src.includes('facebook.com'))
                                ...
                                provider = 'Google';
                            else if (src.includes('doubleclick'))
                ...
                case 'check_privacy_policy': {
                    ...
                    const result = {
                        ...
                        overallScore: Object.values(complianceAnalysis).reduce((sum, compliance) => sum + (compliance.score || 0), 0) / Object.keys(complianceAnalysis).length || 0,
                        analyzedAt: new Date().toISOString(),
                    };
                    return {
                        content: [
                ...
                case 'monitor_uptime': {
                    ...
                    const responseTimes = upChecks.map(c => c.responseTime).filter(rt => rt !== null);
                    ...
                    for (let hour = 0; hour < 24; hour++) {
                        ...
                        const hourResponseTimes = hourUpChecks.map(c => c.responseTime).filter(rt => rt !== null);
                        hourlyStats.push({
                            hour,
                ...
                case 'track_changes_detailed': {
                    ...
                    const structuralChanges = [
                        { element: 'navigation', change: 'Menu item "Products" added', severity: 'minor' },
                        { element: 'footer', change: 'Copyright year updated', severity: 'minor' },
                        { element: 'sidebar', change: 'New widget added', severity: 'moderate' },
                    ];
                ...
                case 'analyze_traffic_patterns': {
                    const { url, timeframe = '30d', includeReferrers = true, } = args;
                    if (!url || typeof url !== 'string') {
                        throw new McpError(ErrorCode.InvalidParams, 'URL parameter is required and must be a string');
                    }
                ...
                case 'generate_reports': {
                    const { url, reportType = 'comprehensive', format = 'json', } = args;
                    if (!url || typeof url !== 'string') {
                        throw new McpError(ErrorCode.InvalidParams, 'URL parameter is required and must be a string');
                    }
                    const fetchResult = await fetchUrl(url, {
...
<body>
    ...
    <div class="section recommendations">
        ...
        <ul>
            ${reportContent.recommendations.map((rec) => `<li>${rec}</li>`).join('')}
        </ul>
    </div>
</body>
</html>`;
...
${reportContent.recommendations.map((rec) => `- ${rec}`).join('\n')}
